...
public class FloodAwareAStar 
{
    ...
    //calcola il path
    public static List<GraphNode> FindFullPath(Graph graph, GraphNode start, GraphNode goal, WaterSystem w, AgentMovement am)
    {
        //nodi da valutare
        ...
        List<GraphNode> tmpPath = new List<GraphNode>();
        ...
        while (open.Count > 0)
        {
            ...
            if(!tmpPath.Contains(current)) tmpPath.Add(current);
            ...
            {   
                ...
                float arrivalTime = am.EstimateTimeToReach(tmpPath.Count-1, tmpPath.IndexOf(start), tmpPath.AsEnumerable().Reverse().ToList() );
                float waterAtArrival = w.GetWaterHeightAt(arrivalTime);

                //se il percorso è migliore lo seleziono
                if (tentativeG < n.gCost && n.height < waterAtArrival)
                {
                    n.gCost = tentativeG;
